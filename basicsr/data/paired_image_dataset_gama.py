...
class Dataset_PairedImage(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            # 在调整图像的gamma值和调整图像饱和度之前，将 numpy 数组转换为 PIL 图像对象
            img_gt = Image.fromarray(img_gt)
            img_lq = Image.fromarray(img_lq)
            # 之前的数据增强方法
            ###########################################################################1
            aug = random.randint(0, 2)  # 随机选择一种数据增强方式
            if aug == 1:
                img_gt = RGBChannelShuffle(img_gt)  # 打乱RGB通道排列顺序
                img_lq = RGBChannelShuffle(img_lq)
            aug = random.randint(0, 2)  # 随机选择一种数据增强方式
            if aug == 1:
                img_gt = TF.adjust_gamma(img_gt, 1)  # 调整图像的gamma值
                img_lq = TF.adjust_gamma(img_lq, 1)
                ########################################################################2
            aug = random.randint(0, 2)  # 随机选择一种数据增强方式
            if aug == 1:
                sat_factor = 1 + (0.2 - 0.4 * np.random.rand())  # 计算饱和度因子
                img_gt = TF.adjust_saturation(img_gt, sat_factor)  # 调整图像饱和度
