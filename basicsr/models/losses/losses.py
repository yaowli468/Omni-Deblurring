...
#         pred_fft = torch.fft.rfft2(x, dim=(-2, -1), norm='backward')
#         target_fft = torch.fft.rfft2(y, dim=(-2, -1), norm='backward')
#
#         loss_fft = self.loss_weight * l1_loss(
...
#
#     def forward(self, pred, target, weight=None, **kwargs):
#         pred_fft = torch.fft.rfft2(pred, dim=(-2, -1), norm='backward')
#         target_fft = torch.fft.rfft2(target, dim=(-2, -1), norm='backward')
#
#         loss_l1_fft = l1_loss(
#             pred, target, weight, reduction=self.reduction) + self.loss_weight * l1_loss(
#             pred_fft, target_fft, weight, reduction=self.reduction)
#
#         return loss_l1_fft
class FFTLoss(nn.Module):
